...
export class AuthService {
  ...
  login(loginuser: UserForLogin) {
    ...
    this.httpClient
      ...
      if(this.userToken)
      this.alertifyService.error("Hatalı giriş");
  }
  register(registerUser: UserForRegister) {
    ...
    this.httpClient
      ...
      .subscribe(data => {
        this.alertifyService.success("Kayıt oldunuz "+registerUser.UserName)
      });
  }
  ...
  getCurrentUserName() {
    return this.helper.decodeToken(this.token).unique_name;
  }
  GetCityNamebyUser() {
    // let a = "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/locality";
    return this.helper.decodeToken(this.token).authmethod;
  }
}
